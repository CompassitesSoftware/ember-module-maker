...
function generateModuleAssets(moduleName, callback) {
  ...
  async.eachSeries(generators, function (generator, asyncCallback) {
    ...
    that.builtModules.push(moduleName);
    callback();
  });
}
...
function buildSingleModule(moduleName, callback) {
  ...
  var logger = this.logger,
      moduleDef, buildModuleFn, generateModuleAssetsFn;

  // dont build module if already built
  if (_.contains(this.builtModules, moduleName)) {
    logger.debug('Module "%s" is already built, skipping.', moduleName);
    return callback();
  }

  moduleDef = this.registry.getModule(moduleName);
  ...
  if (moduleDef.hasDependencies()) {
    logger.debug('Module "%s" has %d dependencies, building them first',
    ...
    logger.debug('Module "%s" has no dependencies.', moduleName);
    generateModuleAssetsFn(moduleName, callback);
  }
}
